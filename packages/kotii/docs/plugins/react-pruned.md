...
> **NOTE** This plugin is a copy of [react](./react.md) plugin. The distinction between the two is that react-pruned is used in production for the same task that react plugin performs. Its source code has been optimised for better handling by nodejs runtime.
