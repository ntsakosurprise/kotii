...
methods.includeScripts = function (preloadedState) {
  ...
  return `
   <script>
     ...
     window.__KOTII_EFFECTS_STATE__ = ${serialize(JSON.stringify(self.effectsData))}
...
methods.doImport = function (toImport) {
  ...
  return new Promise((resolve, reject) => {
    ...
    loadFile(toImport, all, check).then(imported => {
      ...
      self.debug(`importing module:${toImport}, has failed with an error:${err}`);
      reject(err);
    });
  });
};
methods.doKotiiStyles = function () {
  const self = this;
  const jsonStyles = fs.existsSync(`${kotiiKotiiLandPath}${path.sep}dev/styles.json`) ? !process?.useLinkStyleTag ? JSON.parse(fs.readFileSync(`${kotiiKotiiLandPath}${path.sep}dev/styles.json`)) : true : null;
  ...
  if (process?.useLinkStyleTag) {
    ...
    self.styleTags = `<style id="styles-tag">${jsonStyles.toString().replaceAll(",", " ")}</style>`;
  }
};
