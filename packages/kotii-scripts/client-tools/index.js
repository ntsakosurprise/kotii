const SUKU = (function () {
   ...
   *********************************************************************************************************************/
    ...
    getChildby_id: function (parent_tag_selector, child_tag_selector) {
      if (
        typeof parent_tag_selector == null ||
        typeof parent_tag_selector == "string"
      ) {
        throw new Error(
          "One of the required selectors is a string,object expected"
        );
      } else {
        var childElementbyId =
          parent_tag_selector.querySelector(child_tag_selector);
    ...
    getAllChildby_class: function (parent_tag_selector, child_tag_selector) {
      var childElemntsByClass =
        parent_tag_selector.querySelectorAll(child_tag_selector);

      return childElemntsByClass;
    }, // End of getelementBy Class method

    getChildBy_attribute: function (parent_tag_selector, attribute) {
      var childElemntByAttribute = parent_tag_selector.querySelector(
        "[" + attribute + "]"
      );
    ...
    getAllChildBy_attribute: function (parent_tag_selector, attribute) {
      var childElemntsByAttribute = parent_tag_selector.querySelectorAll(
        "[" + attribute + "]"
      );
    ...
    getBy_attribute: function (attribute) {
      var childElemntByAttribute = document.querySelector(
        "[" + attribute + "]"
      );
    ...
    getAllBy_attribute: function (attribute) {
      var childElemntByAttribute = document.querySelectorAll(
        "[" + attribute + "]"
      );
    ...
    get_parent: function (childNode) {
      var childNode = childNode.parentNode;
    ...
    get_child_previous_sibling: function (childNode) {
      if (childNode.parentNode) {
        ...
        if (
          ...
          typeof childNode.previousSibling == null
        ) {
    ...
    replace_child: function (parentNode, replaceNode, refNode) {
      if (parentNode.hasChildNodes()) {
        if (refNode != undefined || refNode != null) {
          ...
          this.throwErrors(
            "A required REFERENCE NODE must be supplied, none is"
          );
        }
      }
    },
    ...
    get_custom_attribute_value: function (element, custAttribute) {
      if (
        ...
        if (
          ...
          this.throwErrors(
            'The "id","class","style","lang","title","dir",' +
        ...
        this.throwErrors(
          "One of the specified arguments is not in the proper format"
        );
      }
    ...
    create_element: function (elTag, attributes) {
      if (elTag && typeof elTag === "string" && elTag != "") {
              ...
              for (attrib in attributes) {
                if (isNaN(attrib)) {
                  ...
                  if (typeof attribValue === "string") {
                    ...
                    this.throwErrors(
                      "Attribute value: " + attribValue + " must be a string"
                    );
                  } // End of valid attribute value check
                } else {
                  this.throwErrors(
                    'All attributes must be of type "String" starting with a letter in the range "A-Z"'
                  );
        ...
        this.throwErrors(
          "Prospective element argument must be a string that is not empty"
        );
      ...
      if (parentNode && parentNode instanceof HTMLElement) {
        if (createText) {
          if (typeof createText === "string") {
            ...
            this.throwErrors(
              "Argument 2 of SUKU.create_text() must be of type String"
            );
          }
        } else {
        ...
        this.throwErrors(
          "Argument 1 of SUKU.create_text() is required and must be an object of type HTMLElement."
        );
      }
    },
    ...
    get_partial_text: function (refTextNode, extractStart, extractCharNum) {
      var extractedText = refTextNode.substringData(
        extractStart,
        extractCharNum
      );
    ...
    normalize_text: function (parentNode) {
      if (parentNode && parentNode instanceof HTMLElement) {
        ...
        var normalizedText = this.get_full_text(
          this.get_first_child(parentNode)
        );

        return normalizedText;
      } else {
        this.throwErrors(
          "Argument 1 of SUKU.normalize_text() must be an object of type HTMLElement"
        );
      }
    },
   ...
   *********************************************************************************************************************/
    ...
    xhr_configuration: function (
      ...
      function loadHandler(event) {
        if (xhr.status === 200 || xhr.status === 304) {
          ...
          var ajax_error =
            "Client Error: The requested resource or url was not found, error message [" +
            xhr.statusText +
            "], error code [" +
            xhr.status +
            "].";
          ...
          var ajax_error =
            "Client Error: Request is not in required format, error message [" +
            xhr.statusText +
            "], error code [" +
            xhr.status +
            "].";
          ...
          var ajax_error =
            "Server Error: The server responded with the technical error code [" +
            xhr.status +
            "]";
    ...
    ajax_methods_validate: function (
      url,
      requestData,
      successCallback,
      failureCallback,
      ...
      if (url && successCallback && failureCallback) {
        if (method != "GET") {
          if (requestData) {
            ...
            switch (method) {
              case "POST":
                this.throwErrors(
                  "Argument 2 of ajax_post() must be none-empty"
                );
                break;
              case "PUT":
              ...
              case "TRACE":
                this.throwErrors(
                  "Argument 2 of ajax_Trace() must be none-empty"
                );
                break;
              default:
                this.throwErrors(
                  "Argument 2 of ajax_delete() must be none-empty"
                );
                break;
            }
        ...
        this.throwErrors(
          "One or more of the required arguments of ajax_get() is either null,empty or not provided"
        );
      ...
      function internalCheck() {
        if (typeof url === "string" && url.trim() != "") {
          if (
            ...
            SUKU.throwErrors(
              "arguments 3 and 4 of ajax_get() must both be functions"
            );
          } // End of required callback functions check
        } else {
          SUKU.throwErrors(
            "argument 1 of ajax_get() must be a none-empty string"
          );
    ...
    ajax_get: function (
      ...
      var validateResult = this.ajax_methods_validate(
        url,
        requestData,
        successCallback,
        failureCallback,
...
const listenToServerEvents = function (evData) {
  ...
  eventSource.onmessage = function (msg) {
    ...
    if (
      ...
      data.event === "kotii-client-css-update"
    ) {
      console.log("UPDATE DATA kotii-client-css-update", data.update);
...
const listenToWebSocketEvents = function () {
  ...
  ws.onmessage = (event) => {
    ...
    try {
      ...
      console.log("THE SOCKET CONNECTION DATA", data);
    } catch (e) {
      return false;
    }
    ...
    if (
      ...
      data.name === "kotii-client-css-update"
    ) {
...
const beginDomUpdate = function (content) {
  ...
  for (let contentItem in content) {
    ...
    if (contentItem === "oldSelectorsUpdate") {
      let selectors = content[contentItem];
      for (
        let selectorIndex = 0;
        ...
        updateDomBySelectorProps(selector.selector, selector.selectorCss);
      }
    } else if (contentItem === "newSelectorsUpdate") {
      let selectors = content[contentItem];
      for (
        let selectorIndex = 0;
        selectorIndex < selectors.length;
        ...
        let selector = selectors[selectorIndex];
        updateDomByNewSelectors(selector.selector, selector.selectorCss);
      }
    } else if (contentItem === "removeSelectorsUpdate") {
      let selectors = content[contentItem];
      for (
        let selectorIndex = 0;
        selectorIndex < selectors.length;
        ...
        let selector = selectors[selectorIndex];
        updateDomByRemovingSelectors(selector);
      }
    } else if (contentItem === "removeSelectorsPropsUpdate") {
      console.log("removeSelectorsPropsUpdate MATCH");
      let selectors = content[contentItem];
      updateDomBySelectorRemoveProps(selectors);
    } else if (contentItem === "removeImportsUpdate") {
  ...
  //   updateDomBySelectorProps(update.key, update)
  // }
};

const updateDomBySelectorProps = function (key, selectorCss) {
  const propsToLoop = selectorCss.propsValue;
  console.log("THE PROPS VALUE", propsToLoop);
  setTargetStylesheet();

  //  let classElements = document.querySelector(key)
  for (let i = 0; i < TARGET_UPDATE_STYLESHEET.cssRules.length; i++) {
    const rule = TARGET_UPDATE_STYLESHEET.cssRules[i];
    console.log("THE CURRENT RULE", rule.selectorText, key);
    if (rule.selectorText === key) {
      for (prop in propsToLoop) {
        console.log("THE PROP", prop, rule.style);
        rule.style.setProperty(prop, propsToLoop[prop]);
      }
...
const updateDomByNewSelectors = function (key, selectorCss) {
  console.log("THE PROPS VALUE", selectorCss);
  let stylesTag = SUKU.getby_id("styles-tag");
  console.log("THE STYLES TAG", stylesTag);
  SUKU.insert_content_inner_end(stylesTag, selectorCss);
};
const updateDomByRemovingSelectors = function (key) {
  setTargetStylesheet();
  for (let i = 0; i < TARGET_UPDATE_STYLESHEET.cssRules.length; i++) {
    const rule = TARGET_UPDATE_STYLESHEET.cssRules[i];

    if (rule.selectorText === key) {
      console.log("THE CURRENT RULE", rule.selectorText, rule);
      TARGET_UPDATE_STYLESHEET.deleteRule(i);
      break;
    }
  }
};
const updateDomBySelectorRemoveProps = function (selectorCss) {
  setTargetStylesheet();
  for (let selectorProp in selectorCss) {
    for (let i = 0; i < TARGET_UPDATE_STYLESHEET.cssRules.length; i++) {
      const rule = TARGET_UPDATE_STYLESHEET.cssRules[i];
      console.log("THE CURRENT RULE", rule.selectorText, selectorProp);
      if (rule.selectorText === selectorProp) {
        let selector = selectorCss[selectorProp];
...
const updateDomByFileRemoval = function (removals) {
  ...
  for (let removeItem = 0; removeItem < removals.length; removeItem++) {
    ...
    console.log(
      "THE NEW ITEM",
      removals[removeItem],
      TARGET_UPDATE_STYLESHEET.textContent
    );
    TARGET_UPDATE_STYLESHEET.ownerNode.textContent =
      TARGET_UPDATE_STYLESHEET.ownerNode.textContent.replace(regex, "");
...
const randomClassStrings = [
  {
    ...
    class: `.test-class {color: brown}`,
  },
  {
    className: ".test-class-2",
    class: `.test-class {color: yellow}`,
  },
  {
    className: ".test-class-3",
    class: `.test-class {color: green}`,
  },
];
let currentClass = null;
const swapClasses = function (classNode) {
  console.log("THE CLASS NODE", classNode);
  let stylesTag = SUKU.getby_id("styles-tag");
  let currentStyleElement = SUKU.getby_id("kotii-test-element");
  let classList = SUKU.get_class_list(currentStyleElement);
  console.log("THE ELEMENT CLASSES", classList);
  console.log("THeStylesTag", stylesTag.textContent);
  if (currentClass) {
    console.log(
      "Index of class",
      SUKU.has_class(classList, currentClass.className)
    );
    if (SUKU.has_class(classList, currentClass.className)) {
      console.log("Class Exist", currentClass);
      SUKU.remove_class(classList, currentClass.className);
      stylesTag.textContent.replace(currentClass.class, "");
    }
  } else {
    currentClass = classNode;
  }

  console.log("THE FIRST CHILD", SUKU.get_first_child(stylesTag));

  SUKU.insert_content_inner_end(stylesTag, classNode.class);
  SUKU.add_class(classList, classNode.className);
};

const initiateFileSwap = function () {
  // setInterval(()=>{
  //   swapClasses(getRandom(randomClassStrings))
  // },60000)
};
...
const getRandom = function (items) {
  ...
  console.log("RANDOM FOUND", random);
  return random;
};
